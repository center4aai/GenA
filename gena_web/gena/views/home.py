...
steps = [
    ...
    ("2. Choose settings",   "Pick question types, select generation and validation models (gate + scoring), then Generate."),
    ("3. Track progress",    "Watch processing status in real time and see model health."),
    ("4. View results",      "Browse generated questions, edit inline, compare versions, and export XLSX."),
]
pages = [
    "views/bot.py",
    "views/bot.py",
...
page_info = [
    (":material/quiz:",        "Data Preprocessing",     "views/bot.py",
...
Chunks are filtered by an automated **gate** before generation; each generated
question is scored with **weighted** validation (decimal totals). Experts can then
review, edit, and approve the results.
""")
